...
### Aggressive Mode (~62-67GB savings)
**Strategy**: Maximum cleanup including historical data
...
- **Space Saved**: ~62-67 GB  
- **Strategy**: Maximum space optimization, some historical queries may fail
...
4. **Combined approach** (prune + compact) can save 117-122GB from original 182GB (64-67%)
...
|------|------------------|--------------------------|-------------------|-------------------|
| **Moderate (Recommended)** | 9 tables (~8.5 GB) | 15 tables (~47+ GB) | None | ~52-57 GB |
| **Aggressive** | 9 tables (~8.5 GB) | 15 tables (~47+ GB) | 2 tables* (~12.5 GB) | ~62-67 GB |
...
- **Real-world impact**: Pruning alone = ~29-37% savings, Pruning + Compaction = ~59-67% savings (from 182GB total)
