...
func RemoteServices(ctx context.Context, cfg *httpcfg.HttpCfg, logger log.Logger, rootCancel context.CancelFunc) (
	db, dbsmt kv.RoDB, eth rpchelper.ApiBackend, txPool txpool.TxpoolClient, mining txpool.MiningClient,
	...
	if cfg.WithDatadir {
		...
		smtKv, err := kv2.NewMDBX(logger).RoTxsLimiter(limiter).Path(cfg.Dirs.Smt).Accede().Open(ctx)
		if err != nil {
			return nil, nil, nil, nil, nil, nil, nil, nil, ff, nil, err
		}
		dbsmt = smtKv
